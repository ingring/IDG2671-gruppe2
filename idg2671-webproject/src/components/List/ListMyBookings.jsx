...
function ListElementMyBookings({tool}) {
    return (
        <a href='./tools/id' className="flex justify-between">
            <div>{tool}</div>
...
function ListMyBookings() {
    ...
    return (
        <List>
            {tools.map((tool) => <ListElement> <ListElementMyBookings tool={tool.tool} /> </ListElement>)}
        </List> 
    )
}
